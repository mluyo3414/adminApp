...
/**
 * 
 * @author Miguel S This activity displays the response from the server. A number
 *         of orders are displayed if available
 */
public class FromServer1 extends Activity
{
	...
	protected void onCreate( Bundle savedInstanceState )
	{
		...
		if ( b != null )
		{
			...
			orderArrayList =
					(ArrayList<HashMap<String, String>>) b.get( "Data" );
	...
	public void updateGUI( ArrayList<HashMap<String, String>> dataFromServer )
	{
		...
		final ListAdapter adapter =
				new SimpleAdapter( this, orderArrayList,
						R.layout.activity_each_order, new String[] {
								"LOCATION", "NAME", "ORDER" }, new int[] {
								R.id.Location, R.id.Name, R.id.Order } );
		...
		orders.setOnItemClickListener( new OnItemClickListener()
		{
					...
					int position, long id )
			{
				AlertDialog.Builder adb =
						new AlertDialog.Builder( FromServer1.this );
				// parsing name
				String[] names =
						orderArrayList.get( position ).toString()
								.split( "NAME=" );
				String name = names[1];
				...
				adb.setPositiveButton( "Ok", new AlertDialog.OnClickListener()
				{
					public void onClick( DialogInterface dialog, int which )
					{
						...
						deletingObject = new deleteObject( FromServer1.this );
						((BaseAdapter) adapter).notifyDataSetChanged();
						deletingObject.execute( orderArrayList.remove(
								positionToRemove ).toString() );
					}
				} );
				adb.show();
